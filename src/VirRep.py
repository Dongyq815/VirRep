...
def get_opts(args):
    ...
    parser.add_argument(
        ...
        default=False,
        action='store_true',
        help='Use automatic mixed precision to accelerate running effeciency.'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Number of threads used to run VirRep on CPU. 0 means to use all available threads.'
    )
...
if __name__ == '__main__':
    ...
    prefix = '.'.join(os.path.basename(options.input_file).split('.')[0:-1])
    outpath_df = os.path.join(
        options.output_dir, options.label+prefix+'_score.tsv'
    )
    pred_df.to_csv(outpath_df, index=False, sep='\t')
    
    outpath_fa = os.path.join(
        options.output_dir, options.label+prefix+'_viruses.fna'
    )
