...
class SeqCls(nn.Module):
        ...
        for i in range(len(config.fc_units)):
            if (i+1) == 1:
                self.classifier.add_module(
                    'linear'+str(i+1),
                    nn.Linear(2*self.concat_features, config.fc_units[i])
                )
            else:
                self.classifier.add_module(
                    'linear'+str(i+1),
                    nn.Linear(config.fc_units[i-1], config.fc_units[i])
                )
                
            self.classifier.add_module(
                'LayerNorm'+str(i+1),
                nn.LayerNorm(config.fc_units[i], eps=config.layer_norm_eps)
            )
    ...
    def forward(
        ...
        input_fw_semantic = input_fw_semantic.view(
            self.config.split*batchsz, self.bert_maxnum_token
        )
        input_rc_semantic = input_rc_semantic.view(
            self.config.split*batchsz, self.bert_maxnum_token
        )
        ...
        input_fw_aln = input_fw_aln.view(
            self.config.split*batchsz, self.aln_maxnum_token
        )
        input_rc_aln = input_rc_aln.view(
            self.config.split*batchsz, self.aln_maxnum_token
        )
        ...
        probs = (self.classifier(embed_fw) + self.classifier(embed_rc)) / 2
...
class SeqclsConfig():
    ...
    def __init__(
        ...
        alnenc_embed_size,
        alnenc_hidden_size,
        alnenc_num_lstm_layers,
        alnenc_lstm_bidirectional,
        fc_units,
        dropout_rate=0.1,
